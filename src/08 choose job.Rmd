...
# 0) PREPARE ENVIRONMENT -----------------------------
    setwd("/Users/durandsinclair/Documents/Training/Hackathons/GovHack 2018 3")
...
#1) GET DATA ---------------------------
    jobs <- read_csv(here("data", "income_by_job.csv"))
    cities <- read_csv(here("data", "city_coordinates.csv"))
    oz_merged <- readRDS(here("data", "consolidated", "oz_merged.rds"))
    
    #job_sectors <- jobs %>% select(training) %>% distinct()
    occupation <- jobs %>% select(occupation) %>% distinct()
...
#List the cities, and allow the user to choose them from a drop down (selectInput)
    ...
    lumpreact = observe({
        if (length(input$job) > 0 ) {
            updated_income <- jobs %>% 
                filter(occupation == input$job)
        }
    })

   numericInput(inputId = "income"
                 , label = "Household income"
                 , value = updated_income$income
                 #, value = 100000
                 #, value = jobs %>% filter(occupation == isolate(input$job)) %>% select(average_income)
                 , min = 0
                 , max = 1000000
                 )
    #final_income = observe({
    #    input$income
    #})
    #observe(input$job, {
    #    xxx <- input$job
    #    updateNumericInput(session = this, "income", value = jobs %>% filter(occupation == input$job) %>% select(average_income) )
    #})
